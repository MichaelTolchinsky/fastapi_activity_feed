...
@router.post(
    "/", response_model=ActivityResponseDTO, status_code=status.HTTP_201_CREATED
)
...
@router.get(
    "/{activity_id}", response_model=ActivityResponseDTO, status_code=status.HTTP_200_OK
)
async def get_activity(
    activity_id: int, activity_service: ActivityService = Depends(get_activity_service)
):
